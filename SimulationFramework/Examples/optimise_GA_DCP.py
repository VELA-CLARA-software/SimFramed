...
class fitnessFunc():
    ...
    def __init__(self, args, tempdir, scaling=4, overwrite=True, verbose=False, summary=False, clean=False):
        ...
        self.framework = Framework(self.dirname, clean=clean)
    ...
    def calculateBeamParameters(self):
        ...
        # try:
            ...
            constraintsListS07 = {
                'tdc_phase_advance': {'type': 'equalto', 'value': (twiss.interpolate(tdc_screen_position,'muy') - twiss.interpolate(tdc_position,'muy')) % 0.25, 'limit': 0, 'weight': 1},
                ...
                'dechirper_sigma_xy': {'type': 'equalto', 'value': 1e3*twiss.interpolate(dechirper_position, 'sigma_y'), 'limit': 1e3*twiss.interpolate(dechirper_position, 'sigma_x'), 'weight': 35},
            }
...
# allbest=[]
# with open('transverse_DCP_best_Short_240_solutions.csv.tmp','r') as infile:
...
if __name__ == "__main__":
    ...
    try:
        print 'best fitness = ', optfunc(hof[0], dir=os.getcwd()+'/transverse_DCP_best_Short_240', scaling=6, overwrite=True, verbose=True, summary=True, clean=True)
