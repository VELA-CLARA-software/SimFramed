...
def create_base_files(scaling):
    ...
    with open('CLARA_best_longitudinal_elegant/CLARA_longitudinal_best_solutions_elegant.csv','r') as out:
        reader = csv.reader(out)
        row1 = next(reader)
    framework.generator.number_of_particles = 2**(3*scaling)
    framework.modifyElement('CLA-L02-CAV', 'field_amplitude', row1[0])
    framework.modifyElement('CLA-L02-CAV', 'phase', row1[1])
    framework.modifyElement('CLA-L03-CAV', 'field_amplitude', row1[2])
    framework.modifyElement('CLA-L03-CAV', 'phase', row1[3])
    framework.modifyElement('CLA-L4H-CAV', 'field_amplitude', row1[4])
    framework.modifyElement('CLA-L4H-CAV', 'phase', row1[5])
    framework.modifyElement('CLA-L04-CAV', 'field_amplitude', row1[6])
    framework.modifyElement('CLA-L04-CAV', 'phase', row1[7])
    framework['bunch_compressor'].set_angle(0.129072605)
...
best = [25*(10**6),-10,25*(10**6),-10,26.56*(10**6),186,27.5*(10**6),0.0,0.107]
# if not post_inj:
...
#               ]
startranges = [[0.96*i, 1.04*i] if abs(i) > 0 else [-20,20] for i in best]
...
if __name__ == "__main__":
    ...
    pop, logbook = opt.eaSimple(pop, toolbox, cxpb=0.5, mutpb=0.2, ngen=30,
    ...
    # try:
    print 'best fitness = ', optfunc(hof[0][0:9], dir=os.getcwd()+'/CLARA_best_longitudinal_elegant', scaling=5, overwrite=True, verbose=True, summary=True, post_injector=post_inj)
