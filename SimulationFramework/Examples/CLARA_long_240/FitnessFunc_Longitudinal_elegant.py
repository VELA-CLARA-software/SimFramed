...
class fitnessFunc():
    ...
    def calculateBeamParameters(self):
        ...
        try:
            ...
            I125Slices = [a for a in self.beam.slice_peak_current if a > 150]
            # if len(I125Slices) < 5:
            #     I125Slices = [0,1000000]
            chirp = self.beam.chirp
            constraintsList = {
                'peakI_min': {'type': 'greaterthan', 'value': abs(peakI), 'limit': 125, 'weight': 100},
                'peakI_max': {'type': 'lessthan', 'value': abs(peakI), 'limit': 200, 'weight': 100},
                'peakIMomentumSpread': {'type': 'lessthan', 'value': peakIMomentumSpread, 'limit': 0.1, 'weight': 2},
                'peakIEmittanceX': {'type': 'lessthan', 'value': 1e6*peakIEmittanceX, 'limit': 0.75, 'weight': 300},
                'peakIEmittanceY': {'type': 'lessthan', 'value': 1e6*peakIEmittanceY, 'limit': 0.75, 'weight': 50},
                'peakIMomentum': {'type': 'equalto','value': 1e-6*peakIMomentum, 'limit': 240, 'weight': 200},
                'sband_linac fields': {'type': 'lessthan', 'value': 1e-6*self.sbandlinacfields, 'limit': 32, 'weight': 50},
                # 'xband_linac fields': {'type': 'lessthan', 'value': 1e-6*self.xbandlinacfields, 'limit': 100, 'weight': 100},
                '4hc field': {'type': 'lessthan', 'value': 1e-6*fhcfield, 'limit': 35, 'weight': 50},
                ...
                # 'chirp': {'type': 'lessthan', 'value': abs(chirp), 'limit': 0.75, 'weight': 5},
                'correct_chirp': {'type': 'equalto', 'value': chirp, 'limit': 0, 'weight': 10},
            ...
            print fitness, 1e-6*peakIMomentum, abs(peakI), np.std(I125Slices), len(I125Slices), 1e6*peakIEmittanceX, 1e6*peakIEmittanceY, sigmat, chirp
            saveState(self.args, fitness, 1e-6*peakIMomentum, abs(peakI), np.std(I125Slices), len(I125Slices), 1e6*peakIEmittanceX, 1e6*peakIEmittanceY, sigmat, chirp)
            return fitness
