...
def analyse_image(mw, dir=None):
    ...
    if dir is not None:
        if os.path.isfile(dir + '/' + 'run.0.gout'):
            ...
            e, b, l, bright, radpower, rx, ry, current, s = analysis(out)
            mb = np.argmax(bright)
            ...
            mb = len([le for le in l if le <= 12.5])

            ax1 = mw.getFigure().add_subplot(421)
            ax1.plot(l, bright)
            ax1.set(ylabel='Power [GW]')
            ax1.grid()
            ax1.axvline(l[mb],color='black',ls='--', alpha=0.5)
            ax1.annotate(np.round(bright[mb], decimals=3), xy=(l[mb], bright[mb]), xytext=(3, 0.5*max(bright)), arrowprops=dict(facecolor='black', alpha=0.5, shrink=0.05))
            ax2 = mw.getFigure().add_subplot(423)
            ax2.plot(l, 1e6*e)
            ax2.set(ylabel=r'Pulse Energy [$\mu$J]')
            ax2.grid()
            ax2.axvline(l[mb],color='black',ls='--', alpha=0.5)
            ax2.annotate(np.round(1e6*e[mb], decimals=3), xy=(l[mb], 1e6*e[mb]), xytext=(3, 0.5*max(1e6*e)), arrowprops=dict(facecolor='black', alpha=0.5, shrink=0.05))
            ax3 = mw.getFigure().add_subplot(425)
            ax3.plot(l, 1e2*b)
            ax3.set(xlabel='z (m)', ylabel='Bandwidth [%]')
            ax3.grid()
            ax3.axvline(l[mb],color='black',ls='--', alpha=0.5)
            ax3.annotate(np.round(1e2*b[mb], decimals=3), xy=(l[mb], 1e2*b[mb]), xytext=(3, 0.5*max(1e2*b)), arrowprops=dict(facecolor='black', alpha=0.5, shrink=0.05))
            # plt.show()
            ...
            ax4.hist2d(1e12*(beam.t - np.mean(beam.t)), 1e-6*beam.cp, bins=(500,500), range=[[-0.2,0.2],[np.mean(1e-6*beam.cp)-20, np.mean(1e-6*beam.cp)+20]], cmap=plt.cm.jet)
            ...
            # print 'beta_x = ', beam.beta_x, ' alpha_x = ', beam.alpha_x
            # print 'beta_y = ', beam.beta_y, ' alpha_y = ', beam.alpha_y
            smod = np.array(1e12*(np.mean(s)-s)/3e8)
            print(smod[-1], x[0])
            smod -= (smod[-1] - x[0])
            ax7.plot(smod, radpower[:,mb]/1e9)
            # ax7.plot(x, beam.slice_beta_x)
            # ax7.plot(x, beam.slice_beta_y)
            # ax7.set_ylim(top=20, bottom=0)
            ax7.set_ylim(top=80, bottom=0)
            ax7.set(ylabel='Power [GW]')
            ax7.grid()
...
class mainApp(qt.QMainWindow):
    ...
    def update_directory_list(self):
        ...
        if os.path.isfile(self.currentDirectory + '/best_solutions_running.csv'):
            ...
            solutions = np.array(sorted([s for s in solutions if s[-7] > 2], key=lambda a: a[-1], reverse=False))
