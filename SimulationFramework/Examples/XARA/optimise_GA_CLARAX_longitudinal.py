...
class fitnessFunc():
    ...
    def __init__(self, args, tempdir, scaling=4, overwrite=True, verbose=False, summary=False):
        ...
        if self.post_injector:
            linac2field, linac2phase, linac3field, linac3phase, fhcfield, fhcphase, linac4field, linac4phase, linac5field, linac5phase, linac6field, linac6phase, bcangle = args
            ...
            gunphase, gunsol, linac1field, linac1phase, linac1sol1, linac1sol2, linac2field, linac2phase, linac3field, linac3phase, fhcfield, fhcphase, linac4field, linac4phase, linac5field, linac5phase, linac6field, linac6phase, bcangle = args
        ...
        if self.post_injector:
            ...
            self.sbandlinacfields = np.array([linac1field, linac2field, linac3field])
        self.xbandlinacfields = np.array([linac4field, linac5field, linac6field])
        ...
        self.framework.modifyElement('CLA-L04-CAV-01', 'field_amplitude', abs(linac4field))
        self.framework.modifyElement('CLA-L04-CAV-01', 'phase', linac4phase)
        self.framework.modifyElement('CLA-L04-CAV-02', 'field_amplitude', abs(linac5field))
        self.framework.modifyElement('CLA-L04-CAV-02', 'phase', linac5phase)
        self.framework.modifyElement('CLA-L04-CAV-03', 'field_amplitude', abs(linac6field))
        self.framework.modifyElement('CLA-L04-CAV-03', 'phase', linac6phase)
    ...
    def calculateBeamParameters(self):
        ...
        try:
            ...
            constraintsList = {
                'peakI_min': {'type': 'greaterthan', 'value': abs(peakI), 'limit': 2000, 'weight': 30},
                'peakI_max': {'type': 'lessthan', 'value': abs(peakI), 'limit': 2500, 'weight': 10},
                'peakIMomentumSpread': {'type': 'lessthan', 'value': peakIMomentumSpread, 'limit': 0.1, 'weight': 2},
                'peakIEmittanceX': {'type': 'lessthan', 'value': 1e6*peakIEmittanceX, 'limit': 1, 'weight': 50},
                'peakIEmittanceY': {'type': 'lessthan', 'value': 1e6*peakIEmittanceY, 'limit': 0.5, 'weight': 50},
                'peakIMomentum': {'type': 'GreaterThan','value': 1e-6*peakIMomentum, 'limit': 800, 'weight': 20},
                ...
                '4hc field': {'type': 'lessthan', 'value': 1e-6*fhcfield, 'limit': 100, 'weight': 100},
                ...
                'momentum_spread': {'type': 'lessthan', 'value': fitp, 'limit': 0.25, 'weight': 2},
                'chirp': {'type': 'lessthan', 'value': abs(chirp), 'limit': 1, 'weight': 0}
            }
...
parameters.append(framework.getElement('CLA-L04-CAV-01', 'field_amplitude'))
parameters.append(framework.getElement('CLA-L04-CAV-01', 'phase'))
parameters.append(framework.getElement('CLA-L04-CAV-02', 'field_amplitude'))
parameters.append(framework.getElement('CLA-L04-CAV-02', 'phase'))
parameters.append(framework.getElement('CLA-L04-CAV-03', 'field_amplitude'))
parameters.append(framework.getElement('CLA-L04-CAV-03', 'phase'))
...
if __name__ == "__main__":
    ...
    try:
        print 'best fitness = ', optfunc(hof[0], dir=os.getcwd()+'/CLARAX_best_longitudinal', scaling=6, overwrite=True, verbose=True, summary=True)
        with open('CLARAX_best_longitudinal/CLARAX_longitudinal_best_solutions.csv','wb') as out:
            csv_out=csv.writer(out)
            for row in hof:
                csv_out.writerow(row)
        with open('CLARAX_best_longitudinal/CLARAX_longitudinal_best_stats.csv','wb') as out:
            csv_out=csv.writer(out)
            for row in stats:
                csv_out.writerow(row)
    except:
        with open('CLARAX_longitudinal_best_solutions.csv.tmp','wb') as out:
            csv_out=csv.writer(out)
            for row in hof:
                csv_out.writerow(row)
