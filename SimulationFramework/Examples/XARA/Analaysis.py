...
class XARA(Optimise_Elegant):
    ...
    def calculate_constraints(self):
        ...
        peakPPDF = self.beam.PDFI(p, p_grid, bandwidth=self.beam.rms(p)/(2**3))
        ...
        if self.plotting and fitness < self.bestfit:
            ...
            ax[0][0].fill_between(t_grid[indexes], peakIPDF[indexes], 0, facecolor='gray', edgecolor='gray', alpha=0.4)
            ax[0][0].set(xlabel='t (ps)', ylabel='I [A]')

            ax[0][1].plot(p_grid, peakPPDF, color='blue', alpha=0.5, lw=3)
            ax[0][1].fill_between(p_grid[indexesp], peakPPDF[indexesp], 0, facecolor='gray', edgecolor='gray', alpha=0.4)
...
if __name__ == "__main__":
    ...
    opt.plotting = True
    # opt.CLARA_dir = os.path.relpath(__file__+'/../../../CLARA/')
    opt.OptimisingFunction(opt.best, dir='XARA_1kA')
    while True:
        plt.pause(0.1)
