    ...
    def calculateBeamParameters(self):
        ...
        try:
            if bcangle < 0.05 or bcangle > 0.12:
                raise ValueError
            self.astra.createInitialDistribution(npart=self.npart, charge=1)
            ...
            if self.summary:
                self.astra.createHDF5Summary(reference='NoCSR_ASTRA_LowQ')
            return fitness
        except:
            return 1e6
...
best = parameters
print optfunc(best, dir=os.getcwd()+'/longitudinal_best_NoCSR_ASTRA_LowQ', npart=2**16, ncpu=20, overwrite=True, verbose=True, summary=True)
exit()
