...
class fitnessFunc():
    ...
    def __init__(self, args, tempdir, npart=10000, ncpu=6, overwrite=True, verbose=False, summary=False):
        ...
        if not os.name == 'nt':
            ...
            # self.astra.defineASTRACommand(['/opt/ASTRA/Astra3.2'])
    ...
    def calculateBeamParameters(self):
        ...
        try:
            ...
            if self.summary:
                if os.name == 'nt':
                    self.astra.createHDF5Summary(reference='NoCSR_ASTRA_Windows')
                else:
                    self.astra.createHDF5Summary(reference='NoCSR_ASTRA_Parallel')
            return fitness
        except:
            return 1e6
...
best = parameters
if os.name == 'nt':
    print optfunc(best, dir=os.getcwd()+'/longitudinal_best_NoCSR_ASTRA_Windows', npart=2**16, ncpu=20, overwrite=True, verbose=True, summary=True)
else:
    print optfunc(best, dir=os.getcwd()+'/longitudinal_best_NoCSR_ASTRA', npart=2**16, ncpu=20, overwrite=True, verbose=True, summary=True)
exit()
