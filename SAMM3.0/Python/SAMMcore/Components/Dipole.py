...
class Dipole(ComponentBase):
    ...
    def Track(self, beam):
        ...
        d1     = sqrt(1 + 2*beam.dp/beam.beta + beam.dp*beam.dp0)
        ...
        if(wy != 0):
            ys  = sinh(wy*self.length)/wy
            ys2 = sinh(2*wy*self.length)/wy
        else:
            ys  = self.length
            ys2 = 2 * self.length
        ...
        c0  = (1/beam.beta - d0/d1)*self.length        \
              - d0*a1*(self.__curvature*(self.length-xs)
              + a1*(2*self.length-xs2)/8)/wx/wx/d1

        c1  =-d0*(self.__curvature*xs - a1*(2*self.length-xs2)/4)/d1

        c2  =-d0*(self.__curvature*(1-xc)/wx/wx + a1*xs*xs/2)/d1/d1
        ...
        beam.ct = ct0 + c0 + c1*beam.x + c2*beam.px +c11*beam.x*beam.x + c12*beam.x*beam.px + c22*beam.px*beam.px \
              + c33*beam.y*beam.y + c34*beam.y*beam.py + c44*beam.py*beam.py
